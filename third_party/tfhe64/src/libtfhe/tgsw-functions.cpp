...
// The upstream AVX2 decomposition path uses 32-bit lanes. Keep Torus64 on the
// scalar decomposition path until a widened vector implementation is added.
#ifdef __AVX2__
#undef __AVX2__
#endif
...
Torus32PolynomialDecompH_old(IntPolynomial *result, const TorusPolynomial *sample, const TGswParams *params) {
    ...
    const uint64_t maskMod = params->maskMod;
    const int32_t halfBg = params->halfBg;
    const uint64_t offset = params->offset;

    for (int j = 0; j < N; ++j) {
        uint64_t temp0 = static_cast<uint64_t>(sample->coefsT[j]) + offset;
        for (int p = 0; p < l; ++p) {
            uint64_t temp1 = (temp0 >> (TFHE_TORUS_BITS - (p + 1) * Bgbit)) & maskMod; // doute
...
//#define __AVX2__ //(to test)
    ...
    const uint64_t offset = params->offset;
#else
    const uint64_t* maskMod_addr = &params->maskMod;
    const int32_t* halfBg_addr = &params->halfBg;
    const uint64_t* offset_addr = &params->offset;
    ...
    for (int p = 0; p < l; ++p) {
        const int decal = (TFHE_TORUS_BITS - (p + 1) * Bgbit);
#ifndef __AVX2__
        int32_t *res_p = result[p].coefs;
        for (int j = 0; j < N; ++j) {
            uint64_t temp1 = (buf[j] >> decal) & maskMod;
