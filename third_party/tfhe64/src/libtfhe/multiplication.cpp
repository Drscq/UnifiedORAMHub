...
inline Torus32 torusFromUint64(uint64_t value) {
    return static_cast<Torus32>(value);
}

inline uint64_t torusMulInt(const int scalar, const Torus32 value) {
    return static_cast<uint64_t>(static_cast<int64_t>(scalar)) * static_cast<uint64_t>(value);
}
...
void torusPolynomialMultNaive_plain_aux(Torus32* __restrict result, const int* __restrict poly1, const Torus32* __restrict poly2, const int N) {
	...
	ri=0;
	for (int j=0; j<=i; j++) {
	    ri += torusMulInt(poly1[j], poly2[i-j]);
	}
	result[i]=torusFromUint64(ri);
    }
...
void torusPolynomialMultNaive_aux(Torus32* __restrict result, const int* __restrict poly1, const Torus32* __restrict poly2, const int N) {
    ...
    for (int i=0; i<N; i++) {
		ri=0;
			...
			for (int j=i+1; j<N; j++) {
		    	ri -= torusMulInt(poly1[j], poly2[N+i-j]);
			}
		result[i]=torusFromUint64(ri);
    }
}
...
EXPORT void torusPolynomialAddMulRNaive(TorusPolynomial* result, const IntPolynomial* poly1, const TorusPolynomial* poly2) {
    const int N = poly1->N;
    assert(poly2->N==N && result->N==N);
    TorusPolynomial* product = new_TorusPolynomial(N);
    torusPolynomialMultNaive(product, poly1, poly2);
    for (int i = 0; i < N; ++i) {
        const uint64_t value = static_cast<uint64_t>(result->coefsT[i]) + static_cast<uint64_t>(product->coefsT[i]);
        result->coefsT[i] = torusFromUint64(value);
    }
    delete_TorusPolynomial(product);
}
