    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < t; ++j) {
            for (int h = 1; h < base; ++h) { // pas le terme en 0
                ...
                // compute the error 
                Torus32 x = (in_key[i]*h)*(Torus32(1) << (TFHE_TORUS_BITS - (j + 1) * basebit));
                temp_err = phase - x;
                // sum all errors 
                error += temp_err;
            }
...
void lweCreateKeySwitchKey_fromArray(LweSample*** result, 
    ...
    for(int i=0;i<n;i++) {
		...
		Torus32 x=(in_key[i]*k)*(Torus32(1) << (TFHE_TORUS_BITS - (j + 1) * basebit));
...
void lweKeySwitchTranslate_fromArray(LweSample* result, 
    ...
    const int base=1<<basebit;       // base=2 in [CGGI16]
    const Torus32 prec_offset=Torus32(1) << (TFHE_TORUS_BITS - (1 + basebit * t)); //precision
    const int mask=base-1;

    for (int i=0;i<n;i++){
	const uint64_t aibar=static_cast<uint64_t>(ai[i]+prec_offset);
	for (int j=0;j<t;j++){
	    const uint64_t aij=(aibar>>(TFHE_TORUS_BITS - (j + 1) * basebit)) & mask;
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < t; ++j) {
            ...
            for (int h = 1; h < base; ++h) { // pas le terme en 0
                ...
                */
                Torus32 mess = (in_key->key[i]*h)*(Torus32(1) << (TFHE_TORUS_BITS - (j + 1) * basebit));
