...
// Idea:
// we represent an element x of the real torus by the integer
// rint(2^64.x) modulo 2^64.  The upstream TFHE API names this type Torus32;
// Onion Ring keeps that API surface while widening the representation.
static const int TFHE_TORUS_BITS = 64;
typedef int64_t Torus64; //avant uint64_t
typedef Torus64 Torus32; //compatibility alias for the upstream API
